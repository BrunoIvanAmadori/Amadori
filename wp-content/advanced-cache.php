...
$possible_plugin_locations = array(
	...
	'C:\Users\Bruno Amadori\Local Sites\geovictorianew\app\public\wp-content\plugins\wp-optimize\cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('http://geovictoria-new.local/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', 'C:\Users\Bruno Amadori\Local Sites\geovictorianew\app\public\wp-content\plugins\wp-optimize\cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-geovictoria-new.local.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-geovictoria-new.local.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-geovictoria-new.local.php');
}
