...
const Condition = ( props ) => {
    ...
    const handleClickDeleteGroup = () => {
        if ( condition.conditions.length ) {
            if ( confirm( __( 'This will delete all conditions in the group.  Are you sure?' ) ) ) {
    ...
    if ( condition.conditions.length ) {
        ...
        groupBody = (
            <p className="wbc-empty-group">{ __( 'This group does not have any conditions yet.' ) }</p>
        )
    }
            ...
            <li className="wbc-group-condition" data-guid={ condition.guid }>
                ...
                <div className="wbc-content">
                    ...
                    <div className="wbc-foot">
                        <Button
            				isLink={ true }
            				onClick={ () => { handleAddCondition( condition ) } }>{ __( 'Add Condition' ) }</Button>
                        <span className="wbc-separator">|</span>
                        <Button
                            isLink={ true }
                            onClick={ handleClickDeleteGroup }>{ __( 'Delete Group' ) }</Button>
                    </div>
                </div>
            </li>
        );
