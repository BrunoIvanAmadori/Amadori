...
class GeotCore {
	...
	public function __construct() {
		...
		if ( ! is_admin()
			...
			if  ( ! empty( $this->opts['cache_mode'] ) || ! empty( $this->opts['wp_rocket_cache'] ) ) {
				add_action( 'init', [ $this, 'getUserData' ] );
			}
			// disable wprocket cookies when  it's turned off
			if (!  empty( $this->opts['wp_rocket_cache'] ) ) {
	...
	public function getUserData( $params = '', $key = 'ip', $force = false ) {
		...
		try {
			...
			// if we are using persistent cache, get the record from there
