...
import {
  elementorCreateParams,
  elementorGetContent,
  elementorInit,
  elementor,
  elementorLoaded,
} from "./addons/elementor";
...
(function () {
  ...
  let ajaxloadmore = function (el, index) {
    ...
    alm.offset = alm.listing.dataset.offset
      ...
      : 0;
    alm.lazy_images = alm.listing.dataset.lazyImages
      ? alm.listing.dataset.lazyImages
      : false;
    ...
    alm.addons.elementor =
      alm.listing.dataset.elementor === "posts" &&
      alm.listing.dataset.elementorSettings
        ? true
        : false;
    ...
    alm.addons.woocommerce =
      alm.listing.dataset.woo && alm.listing.dataset.woo === "true"
        ? true
        : false;
    if (alm.addons.woocommerce && alm.listing.dataset.wooSettings) {
      alm.addons.woocommerce_settings = JSON.parse(
        alm.listing.dataset.wooSettings
      );
      alm.addons.woocommerce_settings.results_text = document.querySelectorAll(
        alm.addons.woocommerce_settings.results
      ); // Add Results Text
      alm.page =
        parseInt(alm.page) + parseInt(alm.addons.woocommerce_settings.paged);
    }
    ...
    if (alm.addons.nextpage === "true") {
      ...
      alm.addons.nextpage_title_template =
        alm.listing.dataset.nextpageTitleTemplate;
    }
    ...
    if (alm.addons.single_post === "true") {
      ...
      alm.addons.single_post_order =
        alm.listing.dataset.singlePostOrder === undefined
          ? "previous"
          : alm.listing.dataset.singlePostOrder;
      alm.addons.single_post_init_id = alm.listing.dataset.singlePostId;
      alm.addons.single_post_taxonomy =
        alm.listing.dataset.singlePostTaxonomy === undefined
          ? ""
          : alm.listing.dataset.singlePostTaxonomy;
      alm.addons.single_post_excluded_terms =
        alm.listing.dataset.singlePostExcludedTerms === undefined
          ? ""
          : alm.listing.dataset.singlePostExcludedTerms;
      alm.addons.single_post_progress_bar =
        alm.listing.dataset.singlePostProgressBar === undefined
          ? ""
          : alm.listing.dataset.singlePostProgressBar;
      alm.addons.single_post_target =
        alm.listing.dataset.singlePostTarget === undefined
          ? ""
          : alm.listing.dataset.singlePostTarget;
      alm.addons.single_post_preview =
        alm.listing.dataset.singlePostPreview === undefined ? false : true;
      if (alm.addons.single_post_preview) {
        const singlePostPreviewData =
          alm.listing.dataset.singlePostPreview.split(":");
        alm.addons.single_post_preview_data = {
          button_label: singlePostPreviewData[0]
            ? singlePostPreviewData[0]
            : "Continue Reading",
          height: singlePostPreviewData[1] ? singlePostPreviewData[1] : 500,
          element: singlePostPreviewData[2]
            ? singlePostPreviewData[2]
            : "default",
    ...
    if (alm.addons.filters === "true") {
      ...
      alm.addons.filters_target = alm.listing.dataset.filtersTarget
        ? alm.listing.dataset.filtersTarget
        : false;
      ...
      if (!alm.addons.filters_target) {
        console.warn(
          "Ajax Load More: Unable to locate target for Filters. Make sure you set a filters_target in core Ajax Load More."
        );
      }
    ...
    alm.max_pages =
      alm.max_pages === undefined || alm.max_pages === 0 ? 9999 : alm.max_pages;
    ...
    /* Scroll Direction */
    alm.scroll_direction =
      alm.scroll_direction === undefined ? "vertical" : alm.scroll_direction;
    ...
    /* Button Labels */
    alm.button_label =
      alm.button_label === undefined ? "Load More" : alm.button_label;
    alm.button_loading_label =
      alm.button_loading_label === undefined ? false : alm.button_loading_label;
    alm.button_done_label =
      alm.button_done_label === undefined ? false : alm.button_done_label;
    ...
    alm.AjaxLoadMore.adminajax = function (alm, action, queryType) {
      ...
      if (alm.addons.single_post && alm.addons.single_post_target) {
        ajaxURL = `${alm.addons.single_post_permalink}?id=${
          alm.addons.single_post_id
        }&alm_page=${parseInt(alm.page) + 1}`;
        params = "";
      }
      ...
      if (is_cache) {
        ...
        let totalposts =
          typeof meta !== "undefined"
            ? meta.totalposts
            : alm.posts_per_page * 5;
        alm.totalposts =
          alm.addons.preloaded === "true"
            ? totalposts - alm.addons.preloaded_amount
            : totalposts;
      ...
      if (total > 0) {
        ...
        if (!alm.addons.paging) {
          ...
          if (alm.addons.single_post) {
            reveal.setAttribute(
              "class",
              `alm-reveal alm-single-post post-${alm.addons.single_post_id}${
                alm.tcc ? ` ${alm.tcc}` : ""
              }`
            );
            ...
            if (
              alm.addons.single_post_preview &&
              alm.addons.single_post_preview_data &&
              typeof almSinglePostCreatePreview === "function"
            ) {
              const singlePreview = window.almSinglePostCreatePreview(
                reveal,
                alm.addons.single_post_id,
                alm.addons.single_post_preview_data
              );
            ...
            if (!alm.transition_container) {
              ...
              reveal =
                alm.container_type === "table"
                  ? tableWrap(alm.html)
                  : stripEmptyNodes(almDomParser(alm.html, "text/html"));
            } else {
              // Standard container
              let pagenum;
                ...
                for (let k = 0; k < return_data.length; k++) {
                  ...
                  if (k > 0 || alm.addons.preloaded === "true") {
                    ...
                    if (alm.addons.filters) {
                      ...
                      alm_reveal.setAttribute(
                        "class",
                        "alm-reveal" + filters_class + preloaded_class + alm.tcc
                      );
                      alm_reveal.dataset.url =
                        alm.canonical_url + buildFilterURL(alm, querystring, 0);
              ...
              // End Init & SEO
                ...
                if (
                  ...
                  reveal.dataset.url =
                    alm.canonical_url +
                    buildFilterURL(alm, querystring, parseInt(alm.page) + 1);
                  reveal.dataset.page = parseInt(alm.page) + 1;
                } else {
                  if (alm.addons.seo) {
                    // SEO [Page 1]
          ...
          if (alm.addons.woocommerce) {
            (async function () {
              ...
              console.log(
                "Ajax Load More: There was an error loading woocommerce products.",
                e
              );
            });
          ...
          if (alm.addons.elementor) {
            (async function () {
              ...
              console.log(
                "Ajax Load More: There was an error loading Elementor items.",
                e
              );
            });
          ...
          if (
            ...
            (alm.init && alm.addons.preloaded !== "true")
          ) {
            if (!isPaged) {
              if (!alm.transition_container) {
                // No transition container.
    ...
    if (
      alm.addons.paging ||
      alm.addons.tabs ||
      alm.scroll_distance_perc ||
      alm.scroll_direction === "horizontal"
    ) {
      let resize;
    ...
    alm.AjaxLoadMore.isVisible = function () {
      ...
      alm.visible =
        alm.main.clientWidth > 0 && alm.main.clientHeight > 0 ? true : false;
      return alm.visible;
    };
          ...
          if (alm.window !== window) {
            ...
            if (alm.scroll_direction === "horizontal") {
              ...
              scrollPosition = Math.round(
                alm.window.scrollLeft +
                  alm.window.offsetWidth -
                  alm.scroll_distance
    ...
    alm.AjaxLoadMore.init = function () {
      ...
      if (alm.addons.nextpage) {
        ...
        if (alm.listing.querySelector(".alm-nextpage") && !alm.addons.paging) {
          ...
          if (nextpage_pages) {
            ...
            let nextpage_total = nextpage_first.dataset.totalPosts
              ? parseInt(nextpage_first.dataset.totalPosts)
              : alm.localize.total_posts;
      ...
      if (alm.addons.woocommerce) {
        ...
        if (
          alm.addons.woocommerce_settings.paged >=
          parseInt(alm.addons.woocommerce_settings.pages)
        ) {
          alm.AjaxLoadMore.triggerDone();
        }
      }
  ...
  if (props && props.type === "woocommerce") {
    ...
    (async function () {
      let instance = document.querySelector(
        '.ajax-load-more-wrap .alm-listing[data-woo="true"]'
      ); // Get ALM instance
      ...
      console.warn(
        "Ajax Load More: There was an resetting the Ajax Load More instance."
      );
    });
  } else {
    // Standard ALM
